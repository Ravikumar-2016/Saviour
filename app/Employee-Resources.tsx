...
const EmployeeResourcesScreen = () => {
  ...
  const [resourceImages, setResourceImages] = useState<{ [id: string]: string }>({})
  ...
  useEffect(() => {
    ...
    // Resources query - only for employee's city
    const resourcesQuery = query(collection(db, "resources"), where("city", "==", employeeCity), orderBy("name", "asc"))
    ...
    // Requests query - only for employee's city
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled && result.assets.length > 0) {
      console.log("Selected new image URI:", result.assets[0].uri)
      setImageUri(result.assets[0].uri)
    }
  }

  // Base64 image storage function
  const storeResourceImageBase64 = async (imageUri: string, resourceId?: string) => {
    try {
      console.log("Starting resource image base64 storage...")
      ...
      return new Promise((resolve, reject) => {
        ...
        reader.onload = async () => {
          try {
            ...
            const imageDocId = resourceId
              ? `resource_${resourceId}`
              : `resource_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`

            console.log("Updating resource image document...")

            // Store in Firestore
            await setDoc(doc(db, "resourceImages", imageDocId), {
            ...
            console.log("Resource image base64 storage successful")
            resolve(imageDocId)
          } catch (error) {
            console.error("Resource image base64 storage failed:", error)
            reject(error)
          }
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (imageUri && !imageUri.startsWith("resource_")) {
        try {
          const imageDocId = await storeResourceImageBase64(imageUri, selectedResource?.id)
          imageUrl = imageDocId
        } catch (error) {
          console.error("Image upload error:", error)
          showSnackbar("Failed to upload image, but resource will be saved without image")
        }
      } else if (imageUri && imageUri.startsWith("resource_")) {
      ...
      if (mode === "add") {
        console.log("Adding new resource document...")
        await addDoc(collection(db, "resources"), resourceData)
        showSnackbar("Resource added successfully")
      } else if (selectedResource) {
        console.log("Updating resource document...")
  ...
  const handleDelete = async () => {
    ...
    Alert.alert("Confirm Delete", `Are you sure you want to delete ${selectedResource.name}?`, [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            if (selectedResource.imageUrl && selectedResource.imageUrl.startsWith("resource_")) {
              try {
                await deleteDoc(doc(db, "resourceImages", selectedResource.imageUrl))
              } catch (error) {
...
const fetchResourceImage = useCallback(async (imageDocId: string) => {
  if (!imageDocId || resourceImages[imageDocId]) return
  try {
    const imageDocRef = doc(db, "resourceImages", imageDocId)
    const imageDocSnap = await getDoc(imageDocRef)
    if (imageDocSnap.exists()) {
      const data = imageDocSnap.data()
      if (data && data.imageData) {
        setResourceImages((prev) => ({ ...prev, [imageDocId]: data.imageData }))
      }
    }
  } catch (error) {
    console.error("Failed to fetch resource image:", error)
  }
}, [resourceImages, setResourceImages, db])

const getResourceImageSource = (resource: Resource) => {
  if (resource.imageUrl && resource.imageUrl.startsWith("resource_")) {
    const base64 = resourceImages[resource.imageUrl]
    if (base64) {
      return { uri: base64 }
    } else {
      // Fetch from Firestore if not cached
      fetchResourceImage(resource.imageUrl)
      return null
    }
  } else if (resource.imageUrl) {
    return { uri: resource.imageUrl }
  }
  return null
}
    ...
    return (
      ...
      >
        ...
        <View style={styles.cardContent}>
          {getResourceImageSource(item) && (
            <Image source={getResourceImageSource(item)!} style={styles.resourceImageModern} />
          )}
            ...
            <View style={styles.stockInfo}>
              <View style={styles.stockNumbers}>
                <Text style={[styles.stockText, { color: colors.text }]}>
                  Available:{" "}
                  <Text style={{ fontWeight: "600", color: isLowStock ? colors.error : colors.success }}>
                    {item.available}
                  </Text>
                </Text>
        ...
        <View style={styles.headerContent}>
          <View>
            <Text style={[styles.headerSubtitle, { color: colors.textSecondary }]}>
              {employeeCity} • {resources.length} Resources • {requests.filter((r) => r.status === "pending").length}{" "}
              Pending
            </Text>
          </View>
          <TouchableOpacity
          ...
          <>
            {["all", "available", "low", "critical"].map((filter) => (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Text style={[styles.filterChipText, { color: resourceFilter === filter ? "#fff" : colors.text }]}>
                    ...
                    <View style={styles.categorySelector}>
                      {RESOURCE_CATEGORIES.map((cat) => (
                        ...
                        >
                          ...
                          />
                          <Text
                            style={[styles.categoryOptionText, { color: category === cat.id ? "#fff" : colors.text }]}
                          >
                            {cat.name}
                          </Text>
                        </TouchableOpacity>
                      ...
                      {DISASTER_RESOURCES[category as keyof typeof DISASTER_RESOURCES].map((resourceName) => (
                        <TouchableOpacity
                          ...
                          style={[
                            ...
                            {
                              backgroundColor:
                                selectedQuickAdd === resourceName ? colors.primary : colors.inputBackground,
                              borderColor: selectedQuickAdd === resourceName ? colors.primary : colors.border,
                            },
                          ]}
                          onPress={() => {
                        ...
                        >
                          <Text
                            style={[
                              styles.quickAddText,
                              { color: selectedQuickAdd === resourceName ? "#fff" : colors.text },
                            ]}
                          >
                            {resourceName}
                          </Text>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
              {mode === "edit" && (
                <TouchableOpacity
                  ...
                  style={[styles.modernButton, styles.deleteButton, { flex: 1, marginRight: 8 }]}
                  disabled={processing}
                  activeOpacity={0.7}
                >
              ...
              <TouchableOpacity
                ...
                style={[styles.modernButton, styles.cancelButton, { flex: 1, marginLeft: mode === "edit" ? 8 : 0 }]}
                disabled={processing}
                activeOpacity={0.7}
              >
              ...
              >
                {processing ? (
                  ...
                  <>
                    ...
                    <Text style={styles.submitButtonText}>{mode === "add" ? "Add Resource" : "Save Changes"}</Text>
                  </>
                )}
              </TouchableOpacity>
            </View>
