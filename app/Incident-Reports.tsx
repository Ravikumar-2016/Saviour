...
export default function AdminIncidentReportsScreen() {
  ...
  const [employeeMap, setEmployeeMap] = useState<{ [uid: string]: { fullName: string; photo: string | null } }>({});
  ...
  // Fetch all incident reports and employee names/photos
  useEffect(() => {
    const fetchReports = async () => {
      ...
      try {
        ...
        // Fetch employee fullName and photo for each employeeId
        const employeeMapTemp: { [uid: string]: { fullName: string; photo: string | null } } = { ...employeeMap };
        await Promise.all(
          employeeIds.map(async (uid) => {
            if (!employeeMapTemp[uid]) {
              try {
                const empDoc = await getDoc(doc(db, "employees", uid));
                if (empDoc.exists()) {
                  const empData = empDoc.data();
                  let photo: string | null = null;
                  if (empData.photoUrl && typeof empData.photoUrl === "string" && empData.photoUrl.startsWith("firestore://")) {
                    // Fetch base64 image from profile_images
                    const parts = empData.photoUrl.split("/");
                    const imageId = parts[parts.length - 1];
                    try {
                      const imgDoc = await getDoc(doc(db, "profile_images", imageId));
                      if (imgDoc.exists() && imgDoc.data().imageData) {
                        photo = imgDoc.data().imageData;
                      }
                    } catch {}
                  }
                  employeeMapTemp[uid] = {
                  ...
                  employeeMapTemp[uid] = { fullName: "Unknown", photo: null };
                }
              } catch {
                employeeMapTemp[uid] = { fullName: "Unknown", photo: null };
              }
  ...
  const filteredReports = reports.filter((report) => {
    ...
    const emp = employeeMap[report.employeeId] || {};
    return (
  ...
  // Helper to get employee name and photo (only if not anonymous)
  const getEmployeeName = (report: any) => {
    if (report.anonymous) return "Unknown";
    const emp = employeeMap[report.employeeId];
    return emp && emp.fullName ? emp.fullName : "Unknown";
  };
  const getEmployeePhoto = (report: any) => {
    if (report.anonymous) return null;
    const emp = employeeMap[report.employeeId];
    return emp && emp.photo ? emp.photo : null;
  };
                ...
                >
                  <View style={s.cardHeader}>
                    <View style={{ flexDirection: "row", alignItems: "center" }}>
                      {getEmployeePhoto(report) && (
                        <Image
                          source={{ uri: getEmployeePhoto(report) ?? "" }}
                          style={{
                            width: 28,
                            height: 28,
                            borderRadius: 14,
                  ...
                  <View style={s.cardFooter}>
                    ...
                    {/* Show image icon if there are images */}
              ...
              <>
                ...
                <View style={s.modalSection}>
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center", marginTop: 4 }}>
                    {getEmployeePhoto(selectedReport) && (
                      <Image
                        source={{ uri: getEmployeePhoto(selectedReport) ?? undefined }}
                        style={{
                          width: 32,
                          height: 32,
                          borderRadius: 16,
                ...
                {/* Show all images if photoUrls exist */}
                {Array.isArray(selectedReport.photoUrls) && selectedReport.photoUrls.length > 0 && (
                  <View style={s.modalSection}>
                    <ThemedText style={s.modalLabel}>Images:</ThemedText>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 8 }}>
                      {selectedReport.photoUrls.map((url: string, idx: number) => (
                        <Image
                          key={idx}
                          source={{ uri: url }}
                          style={{
