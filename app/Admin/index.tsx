...
export default function AdminHomeScreen() {
  ...
  const [employeeStats, setEmployeeStats] = useState({ total: 0, onDuty: 0 });
  ...
  // Fetch user and employee stats (city-restricted, real-time)
  useEffect(() => {
    if (!adminProfile?.city) return;
    // Users
    const usersQ = query(
      collection(db, "users"),
      where("role", "==", "user"),
      where("city", "==", adminProfile.city)
    );
  ...
  // Fetch recent requests (resource or SOS, city-restricted)
  ...
  const getWeatherGradient = () => {
    ...
    if (weatherMain.includes("sun") || weatherMain.includes("clear"))
        ...
        <Animated.View entering={FadeIn.delay(300).duration(600)}>
          ...
          >
            ...
            >
              ...
              <Text style={{ color: "#fff", fontSize: 14 }}>
                On Duty Now: {employeeStats.onDuty}
              </Text>
            </LinearGradient>
          </View>
        </Animated.View>
